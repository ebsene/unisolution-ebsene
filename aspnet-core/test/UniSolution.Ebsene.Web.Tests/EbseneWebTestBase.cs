...
{
    public abstract class EbseneWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static EbseneWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(EbseneWebMvcModule).Assembly.FullName);
        }
        ...
        protected void UsingDbContext(Action<EbseneDbContext> action)
        {
            using (var context = IocManager.Resolve<EbseneDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected T UsingDbContext<T>(Func<EbseneDbContext, T> func)
        {
            T result;
        ...
        protected async Task UsingDbContextAsync(Func<EbseneDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<EbseneDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<EbseneDbContext, Task<T>> func)
        {
            T result;
